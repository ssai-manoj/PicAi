...
//
    ...
    @objc private func handlePinchGesture(_ gesture: UIPinchGestureRecognizer) {
        ...
        case .changed:
            ...
            let clampedZoomFactor = max(1, min(newZoomFactor, camera.maxAvailableVideoZoomFactor))
            
            print("clampedZoomFactor: ", clampedZoomFactor)
            
            if clampedZoomFactor <= 2
            {
                ...
                let ultraWideCamera = AVCaptureDevice.DiscoverySession(deviceTypes: [.builtInUltraWideCamera], mediaType: .video, position: .unspecified).devices.first
